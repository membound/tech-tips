...
```
У цьому прикладі:
*   <code v-pre>`{{ .Values.someValue }}`</code> звертається до значень у `values.yaml`.
*   <code v-pre>`{{ .Release.Name }}`</code> використовує ім'я, надане під час встановлення (`helm install my-release .`).
*   <code v-pre>`{{ .Chart.Name }}`</code> використовує ім'я чарту з `Chart.yaml`.
*   Конструкція <code v-pre>`{{- if ... }} ... {{- end }}`</code> дозволяє умовно включати частини шаблону.
*   <code v-pre>`{{- ... -}}`</code> видаляє зайві пробіли навколо тегу.
:::
