...
5.  **Встановлення TCP З'єднання**
    ...
    *   **Крок 1 (SYN):** Клієнт надсилає пакет SYN (Synchronize) серверу, вказуючи свій початковий номер послідовності.
    *   **Крок 2 (SYN-ACK):** Сервер відповідає пакетом SYN-ACK, підтверджуючи отримання SYN клієнта (ACK) і надсилаючи свій власний SYN зі своїм початковим номером послідовності.
    *   **Крок 3 (ACK):** Клієнт надсилає пакет ACK, підтверджуючи отримання SYN-ACK від сервера. **Цей крок критично важливий, оскільки він повідомляє серверу, що клієнт отримав підтвердження і обидві сторони готові до обміну даними.**
    ...
    TCP розроблений з урахуванням можливої втрати пакетів:

    *   **Якщо SYN-ACK (Крок 2) втрачено:** Клієнт не отримає відповіді на свій SYN. Після певного тайм-ауту він повторно надішле пакет SYN.
    *   **Якщо ACK (Крок 3) втрачено:** Сервер не отримає підтвердження свого SYN-ACK. Після певного тайм-ауту він повторно надішле пакет SYN-ACK. Клієнт, отримавши дублікат SYN-ACK, просто надішле ще один ACK.
    ...
    ::: info Чому не потрібен 4-й крок?
    Після отримання сервером фінального ACK (Крок 3), *обидві* сторони знають, що з'єднання встановлено і синхронізовано. Клієнт дізнався про готовність сервера на Кроці 2 (отримавши SYN-ACK), а сервер дізнався про готовність клієнта на Кроці 3 (отримавши ACK). Додавання 4-го кроку (підтвердження отримання ACK) було б надлишковим для *встановлення* з'єднання і могло б призвести до нескінченного циклу підтверджень. Надійність передачі *самих даних* після встановлення з'єднання забезпечується іншими механізмами TCP (підтвердженнями ACK на пакети даних).
    :::

6.  **TLS/SSL Рукостискання (для HTTPS)**
    ...
    *   хеш-функції/MAC (наприклад, SHA-256) для перевірки цілісності даних.
    ...
    ```
    **Пояснення діаграми:**
    *   Спочатку клієнт та сервер обмінюються інформацією про підтримувані методи шифрування та випадковими даними.
...
10. **Обробка Відповіді Браузером**
...
11. **Завантаження Додаткових Ресурсів**
...
12. **Рендеринг Сторінки / Оновлення Інтерфейсу**
...
13. **Відображення/Оновлення Сторінки**
